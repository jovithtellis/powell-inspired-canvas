...
const Navbar = () => {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 py-4 px-4 md:px-8 ${isScrolled ? 'bg-background/90 backdrop-blur-md shadow-sm' : 'bg-transparent'}`}>
      ...
      {isScrolled && (
        <div className="absolute bottom-0 left-0 right-0">
          <Separator className="separator-silver" />
        </div>
      )}
    </nav>
  );
